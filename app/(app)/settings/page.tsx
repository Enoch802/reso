...
function ScreenTimeModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const saveGoal = async (minutes: number) => {
    const clamped = Math.max(30, Math.min(1440, Math.round(minutes)));
...
function AlarmsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const syncNative = async () => {
    ...
    await syncNativeAlarms(nativeAlarms);
  };
  ...
  const saveEdit = async () => {
    if (!editing) return;
    await db.alarms.update(editing.id, { label: editing.label.trim(), time: editing.time });
    setEditing(null);
    await syncNative();
  };
          ...
          {alarms.map((a) => (
            <div key={a.id} className="flex items-center justify-between rounded-xl neo-sm px-4 py-3">
                ...
                <div className="flex-1 flex flex-wrap gap-2 items-end mr-2">
                  ...
                  <NeoButton variant="accent" className="font-semibold" onClick={() => void saveEdit()}>Save</NeoButton>
                  <NeoButton onClick={() => setEditing(null)}>Cancel</NeoButton>
                </div>
              ) : (
                <>
                  <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2.5 shrink-0">
                    <NeoSwitch
                      ...
                      onChange={async (v) => {
                        await db.alarms.update(a.id!, { enabled: v ? 1 : 0 });
                        await syncNative();
                      }}
                    />
                    <button
                      onClick={() => setEditing({ id: a.id!, label: a.label, time: a.time })}
                    ...
                    <button
                      onClick={async () => { await db.alarms.delete(a.id!); await syncNative(); }}
